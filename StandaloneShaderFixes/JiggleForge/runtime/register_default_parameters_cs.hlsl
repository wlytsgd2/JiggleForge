...
Buffer<uint> StateIndices : register(t0);
Buffer<float4> SourceParameters : register(t1);
RWBuffer<float4> GlobalParameters : register(u0);

[numthreads(64, 1, 1)]
void main(uint3 dispatchThreadId : SV_DispatchThreadID)
{
    uint stateListIndex = dispatchThreadId.x;
    uint stateCount;
    StateIndices.GetDimensions(stateCount);
    if (stateListIndex >= stateCount)
    {
        return;
    }
    ...
    uint stateIndex = StateIndices[stateListIndex];
