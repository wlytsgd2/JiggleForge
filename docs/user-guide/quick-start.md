...
Opening a folder containing `JiggleForge.txt` loads the saved project. After installing a new global runtime, apply the project once so its generated sections use the current runtime contract. Keep a backup before changing a project created by an older build.
