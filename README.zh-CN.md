...
3. 打开“运行环境”，选择 ZZMI 根目录。
4. 点击“安装或更新运行时”，回到游戏按 `F10`。
5. 将替换 Mod 文件夹拖入应用，或使用文件夹选择器打开。
6. 在 Draw 页面检查 Draw、分组、Mask 和检测器开关。
7. 需要时在“物理参数”和“依赖关系”页面修改配置，然后点击“应用配置”。
8. 回到游戏按 `F10` 重新加载生成资源。
9. 按住配置好的拖动键拖动模型。需要滚轮控制深度时，在“运行环境”页面启动 WheelBridge。
